...
--   sRGB color space.
fromRGB' :: Float -> Float -> Float -> Spectrum
fromRGB' r g b = rgbToSpectrumRefl (r, g, b) -- r g b = Spectrum $ V.fromList [r, g, b]

rgbToSpectrumRefl :: (Float, Float, Float) -> Spectrum
rgbToSpectrumRefl (r, g, b)
   | r <= g && r <= b =
      sScale rgbReflWhite r + if g <= b
         then sScale rgbReflCyan (g - r) + sScale rgbReflBlue  (b - g)
         else sScale rgbReflCyan (b - r) + sScale rgbReflGreen (g - b)
   |  g <= r && g <= b =
      sScale rgbReflWhite g + if r <= b
         then sScale rgbReflMagenta (r - g) + sScale rgbReflBlue (b - r)
         else sScale rgbReflMagenta (b - g) + sScale rgbReflRed  (r - b)
   | otherwise =
      sScale rgbReflWhite g + if r <= b
         then sScale rgbReflYellow (r - b) + sScale rgbReflGreen (g - r)
         else sScale rgbReflYellow (g - b) + sScale rgbReflRed   (r - g)
...
rgbFunc :: [Float] -> Spd
rgbFunc = mkSpd' rgbToSpectrumStart rgbToSpectrumEnd

rgbReflWhite :: Spectrum
rgbReflWhite = fromSpd $ rgbFunc
...
rgbReflMagenta = fromSpd $ rgbFunc
